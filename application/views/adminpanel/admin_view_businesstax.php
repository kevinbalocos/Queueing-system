...
<body class="bg-gray-100">
  ...
  <div class="flex h-screen p-5 pt-20">
              ...
              <div class="flex justify-between items-center mt-auto pt-4 border-t">
                <div class="flex space-x-2">
                  <button
                    ...
                    data-id="<?= $item->id; ?>"
                    data-url="<?= base_url("controller_queueing/proceed_to_payment/{$item->id}") ?>"
    ...
    <div class="ml-5 bg-white p-5 w-[400px] rounded-lg shadow-md flex flex-col">
      ...
      <ul id="queueList" class="mt-3 overflow-auto h-[600px] space-y-3 p-2 bg-white rounded-lg shadow-md border">
          ...
          <li
            ...
            <div class="flex items-center gap-4">
              ...
              <div class="flex flex-col">
                <div class="text-lg font-semibold text-cyan-900"><?= $item->name; ?></div>
                <span class="text-xs text-gray-500"><?= date('M d, Y, h:i A', strtotime($item->created_at)); ?></span>
              </div>
            </div>
      ...
      <form action="<?= base_url('controller_queueing/add_to_businesstax'); ?>" method="post" class="mt-5">
        ...
        <select name="reason" required class="border p-2 w-full rounded mt-2 text-cyan-800 font-semibold tracking-wide">
          <option class="text-cyan-800 font-semibold tracking-wide" value="Income Tax Filing">Income Tax Filing</option>
          <option class="text-cyan-800 font-semibold tracking-wide" value="Business Registration">Business Registration
          </option>
        </select>
        <button type="submit"
  ...
  <script>
    ...
    socket.onopen = () => console.log("Connected to WebSocket server (Business Tax)");
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (data.action === "proceed_to_businesstax") {
          ...
          addQueueItem(data, "Business Tax");
