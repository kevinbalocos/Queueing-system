...
<body class="bg-gray-100">
  ...
  <script>
    ...
    socket.onopen = () => console.log("Connected to WebSocket server (Examiners)");
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (data.action === "proceed_to_examiners") {
          console.log("New Examiners queue item received:", data);
          addQueueItem(data, "Examiners");
    ...
    function createQueueItem(data, currentUser) {
      ...
      const proceedUrl = `http://localhost/OJT/queueing-system/index.php/controller_queueing/proceed_to_businesstax/${data.queue_id}`;
      ...
      const processingText = isProcessing ? `Processing by ${data.processing_by}` : "Examiners";
  ...
  `;
    ...
    `;
        ...
        listItem.setAttribute("data-proceed_url", `http://localhost/OJT/queueing-system/index.php/controller_queueing/proceed_to_businesstax/${data.queue_id}`);
    ...
    function moveFirstRightItemToLeft() {
      ...
      if (queueContainer.children.length < 20 && queueList.children.length > 0) {
        ...
        tempDiv.innerHTML = createQueueItem(queueData, "Examiners");

        const newQueueItem = tempDiv.firstElementChild;
        firstRightItem.remove();
        queueContainer.appendChild(newQueueItem);

        console.log(`✅ Moved queue item (ID: ${queueData.queue_id}) to the left preserving original timestamp.`);
      }
    }
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      socket.onmessage = function (event) {
        try {
          ...
          if (data.action === "update_queue") {
            ...
            // ✅ Disable ALL processing buttons for this queue
            document.querySelectorAll(`.processing-btn[data-id="${data.queue_id}"]`).forEach((btn) => {
      ...
      document.body.addEventListener("click", function (e) {
        ...
        if (button) {
          ...
          // ✅ Disable the button for this user immediately
          button.classList.add("opacity-50", "cursor-not-allowed");
          button.setAttribute("disabled", "disabled");

          fetch("<?= base_url('controller_queueing/mark_as_processing_examiners'); ?>", {
            method: "POST",
            ...
            .then(data => {
              if (data.status === "success") {
                ...
                // ✅ Send WebSocket update so other users disable their buttons
