...
<body class="bg-gray-100">
  <div class="flex min-h-screen p-5">
    ...
    <div class="flex-1 bg-white p-5 rounded-lg shadow-md flex flex-col">
      <h2 class="text-2xl font-bold text-blue-900 text-center">Now Serving - Business Tax</h2>
      <?php if ($businesstax): ?>
      ...
      <?php else: ?>
        <p class="text-gray-500 mt-3 text-center">No one in business tax queue</p>
      <?php endif; ?>
    </div>
    ...
    <div class="ml-5 bg-white p-5 w-[400px] rounded-lg shadow-md flex flex-col">
      ...
      <form action="<?= base_url('controller_queueing/add_to_businesstax'); ?>" method="post" class="mt-5">
        ...
        <button type="submit" class="mt-2 w-full bg-green-500 text-white py-2 rounded">Add to Business Tax Queue</button>
      </form>
    </div>
  </div>
</body>
