  ...
  <script>
    ...
    function addNewExaminersQueue(data) {
      ...
      if (!queueContainer || !queueList) {
        console.error("❌ Queue containers not found.");
        return;
      }
      ...
      const statusText = isProcessing ? `Processing by ${data.processing_by}` : "Examiners";
      const statusColor = isProcessing ? "text-red-500" : "text-green-500";
      const proceedDisabled = isProcessing ? "disabled opacity-50 cursor-not-allowed" : "";
      ...
      listItem.dataset.proceed_url = data.proceed_url
        ? data.proceed_url
        : `http://localhost/OJT/queueing-system/index.php/controller_queueing/proceed_to_businesstax/${data.queue_id}`;
  ...
  `;
    ...
    function moveFirstRightItemToLeft() {
      ...
      if (queueContainer.children.length < 20 && queueList.children.length > 0) {
        ...
        const processingText = isProcessing ? `Processing by ${processingBy}` : "Examiners";
        const statusColor = isProcessing ? "text-red-500" : "text-green-500";
